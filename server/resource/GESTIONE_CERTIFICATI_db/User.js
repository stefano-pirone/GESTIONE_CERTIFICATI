...
app.post(properties.api + '/Users', function (req, res) {
	obj = new db_GESTIONE_CERTIFICATI_db.User(req.body);
...
app['delete'](properties.api + '/Users/:id', function (req, res) {
	db_GESTIONE_CERTIFICATI_db.User.findByIdAndRemove(req.params.id, function (err) {
		  if (err) return handleError(err, res);
		  res.send(err);
	});
});
...
app.get(properties.api + '/Users/:id', function (req, res) {
	db_GESTIONE_CERTIFICATI_db.User.findOne({_id:req.params.id}).exec(function (err, obj) {
		if (err) return handleError(err, res);
		res.send(obj);
	});
});
...
app.get(properties.api + '/Users', function (req, res) {
	db_GESTIONE_CERTIFICATI_db.User.find().exec(function (err, list) {
		if (err) return handleError(err, res);
		res.send(list);
	});
});
...
app.post(properties.api + '/Users/:id', function (req, res) {
	db_GESTIONE_CERTIFICATI_db.User.findByIdAndUpdate(req.params.id, req.body, {'new': true}, function (err, obj) {
		if (err) return handleError(err, res);
		res.send(obj);
	});
});
