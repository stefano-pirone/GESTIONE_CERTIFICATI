...
app.post(properties.api + '/changePassword', function(req, res) {
    ...
    db_GESTIONE_CERTIFICATI_db.User.findOne({ username: req.user.username, password: req.body.passwordOld }).exec(function(err, user) {
...
var getUser = function(username, password, callback) {
    ...
    db_GESTIONE_CERTIFICATI_db.User.findOne({ username: username, password: password }).lean().exec(function(err, user) {
        if (user)
            user.password = undefined;
        callback(user);
    });
...
var createUser = function() {
    ...
    db_GESTIONE_CERTIFICATI_db = require('../db/GESTIONE_CERTIFICATI_db_schema.js');
    db_GESTIONE_CERTIFICATI_db.User.count().exec(function(err, count) {
        if (count == 0) {
            var admin = new db_GESTIONE_CERTIFICATI_db.User({
                username: 'admin',
